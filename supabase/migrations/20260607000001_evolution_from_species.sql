-- Add `from_species_id` (the parent's national dex number) to each evolution node
-- in the pokemon_detail view, so the client can distinguish a linear chain from a
-- branching one (e.g. Eevee → Vaporeon/Jolteon/Flareon) and render the fan faithfully.
-- Recreates the view (security_invoker, unchanged otherwise).
